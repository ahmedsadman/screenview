...
const UsersPage = ({ match }) => {
  ...
  const getMovieDetails = async (watchList) => {
    ...
    let movieDetails = watchList.map((item) =>
      api.getMediaDetails(item.mediaId)
    );
        ...
        {showFollowing && (
          <div className="flex justify-center dark:bg-gray-800">
            <h2 className="my-4 text-2xl font-bold">
              {' '}
              You are {!isFollowing ? 'not' : ''} following {user.name}{' '}
            </h2>
            <button className="ml-4">
              {!isFollowing ? (
                <PersonAddIcon
                  ...
                  onClick={() => handleFollowUnfollow('follow', id)}
                />
              ) : (
                <PersonAddDisabledIcon
                  fontSize="large"
                  className="text-red-300 hover:text-red-500"
                  onClick={() => handleFollowUnfollow('unfollow', id)}
                />
              )}
            </button>
          </div>
