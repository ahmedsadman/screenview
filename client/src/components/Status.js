          ...
          <div className="bg-white w-full shadow-lg border-2 border-gray-100 rounded-lg p-5">
            <div className="flex items-center">
              <Link
                ...
                className="hover:bg-gray-100 p-2 items-center flex rounded justify-center"
              >
                <img
              ...
              <div>
                <Link
                  to={`/user/${post.author._id}`}
                  className="flex items-center justify-between w-full py-2 rounded ml-4"
                >
                  <h4 className="text-lg text-gray-700 hover:underline">
                    {post.author.name}
                  </h4>
                </Link>
            ...
            <div>
              ...
              <div
                className="flex relative shadow-md"
                onMouseEnter={() => handleMouseIn(media.id)}
              ...
              >
                ...
                {hoveredId && hoveredId === media.id ? (
                  <div className="has-tooltip">
                    <span className="tooltip rounded shadow-lg bg-white-100 text-xs mt-8 text-center -ml-10">
                      Add To Watch List
                    </span>
                    <button
                      className="flex absolute right-2 top-2 justify-start rounded-md text-gray-400 hover:text-gray-800 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-indigo-500"
                      onClick={() =>
                        _addToWatchList(media.title, 'movie', media.id)
                      }
                    >
            ...
            <div className="flex justify-center">
              <div className="bg-white w-full mb-4 mt-2 flex flex-row flex-wrap justify-center">
                <button
                  className="w-full p-2 hover:bg-gray-100 hover:text-gray-800 flex justify-center rounded-md items-center text-xl text-gray-500 font-semibold"
                  onClick={makeComment}
                >
            ...
            {commentArea ? (
              <div className="rounded-lg bg-gray-50 mb-2 shadow">
                <form action="#" method="post">
                  <textarea
                    className="bg-gray-100 w-full rounded-t-lg border"
                    rows="3"
            ...
            {commentVisibility ? (
              <div className="w-full">
                ...
                <button
                  className="text-xs mt-2 text-gray-400 hover:underline hover:text-gray-500"
                  onClick={showAllComment}
                >
                  View All Comments ({post.totalComments[0].count})
                </button>
              </div>
            ) : (
              <p className="text-xs mt-2 text-gray-400">
                Wow Such Empty. Be the first to comment
              </p>
            )}
